...
class DatabaseHealth(ComponentHealth):
    ...
    connection_pool_active: Optional[int] = Field(None, description="Active database connections")
    connection_pool_idle: Optional[int] = Field(None, description="Idle database connections")
    query_time_avg_ms: Optional[float] = Field(None, description="Average query time in milliseconds")
...
class ModelHealth(ComponentHealth):
    ...
    last_inference_time: Optional[datetime] = Field(None, description="Last inference timestamp")
    inference_count: int = Field(0, description="Total inference count")
    average_inference_time_ms: Optional[float] = Field(None, description="Average inference time in milliseconds")
...
class MLXHealth(ComponentHealth):
    ...
    unified_memory_gb: Optional[float] = Field(None, description="Unified memory available in GB")
...
class DetailedHealthResponse(BaseModel):
    ...
    mlx: Optional[MLXHealth] = Field(None, description="MLX framework health")
    database: Optional[DatabaseHealth] = Field(None, description="Database health")
    
    # Performance metrics
    requests_per_second: Optional[float] = Field(None, description="Current requests per second")
