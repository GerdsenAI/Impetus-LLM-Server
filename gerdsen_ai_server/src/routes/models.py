...
def _load_model_internal(model_id: str, app_state: dict) -> dict:
    ...
    # Estimate required memory from model directory size on disk
    model_dir = settings.model.models_dir / model_id
    if model_dir.exists():
        total_bytes = sum(f.stat().st_size for f in model_dir.rglob("*") if f.is_file())
        # MLX models need ~1.2x disk size in RAM
        required_gb = (total_bytes / (1024**3)) * 1.2
    else:
        required_gb = 4.0  # conservative default

    # Use available memory with a 2GB safety buffer
    if available_gb < required_gb + 2.0:
        error_resp = ErrorResponse.insufficient_memory(required_gb, available_gb)
        return {
...
def list_models():
    ...
    try:
        ...
        for model in models:
            ...
            if model_id in model_benchmarks:
                model["benchmark"] = {
                    ...
                    "tokens_per_second": model_benchmarks[model_id][
        ...
        return jsonify(
            {"models": models, "models_directory": str(settings.model.models_dir)}
        )
    except Exception as e:
    ...
    if auto_warmup:
        ...
        try:
            ...
            model = loader.load_model(
                model_id, auto_warmup=True, warmup_async=True, use_mmap=use_mmap
            )
            ...
            return jsonify(
                {
                    ...
                    "warmup": {
                        "is_warmed": (
                            warmup_status.is_warmed if warmup_status else False
                        ),
                        "status": (
                            "warming"
                            if auto_warmup
                            and warmup_status
                            and not warmup_status.is_warmed
                            else "not_started"
                        ),
                    },
                }
        ...
        if "error" in result:
            ...
            return (
                jsonify({"error": result["error"], "message": result["message"]}),
                status_code,
            )
        else:
...
def unload_model():
    ...
    try:
        ...
        return jsonify({"error": "Failed to unload model", "message": str(e)}), 500
...
def download_model():
    ...
    estimated_size = (
        download_manager.get_download_size(model_id) or 5.0
        if not model_info
            ...
            with app.app_context():
                ...
                if success and auto_load:
                    ...
                    if "error" in load_result:
                        # Auto-load failed
                        logger.error(
                            f"Auto-load failed for {model_id}: {load_result['message']}"
                        )
                        if socketio:
                            socketio.emit(
                                "auto_load_failed",
                        ...
                        if socketio:
                            socketio.emit(
                                "auto_load_complete",
                                {
                                    ...
                                    "message": load_result["message"],
                                    "memory_used_gb": load_result.get(
                                        "memory_used_gb", 0
                                    ),
                                },
                                room=f"download_{task_id}",
                            ...
                            loaded_models = list(
                                app_state.get("loaded_models", {}).keys()
                            )
                            socketio.emit(
                                "models_update",
...
def discover_models():
    ...
    return jsonify({"models": results, "total": len(results), "hardware": chip_type})
    ...
    return jsonify(
        {
            ...
            "downloaded_gb": (
                task.downloaded_bytes / (1024**3) if task.downloaded_bytes else 0
            ),
...
def list_downloads():
    ...
    return jsonify({"downloads": results, "total": len(results)})
        ...
        return jsonify(
            {
                ...
                "summary": {
                    "average_tokens_per_second": round(
                        suite.average_tokens_per_second, 1
                    ),
                    "average_first_token_latency_ms": round(
                        suite.average_first_token_latency_ms, 1
                    ),
                ...
                "history": [
                    {
                        ...
                        "peak_tokens_per_second": round(
                            suite.peak_tokens_per_second, 1
                        ),
    ...
    try:
        ...
        for row in summary:
            ...
            if model_id not in models:
                models[model_id] = {"model_id": model_id, "chips": {}}
        ...
        return jsonify({"models": list(models.values()), "total_models": len(models)})
...
def warmup_model(model_id: str):
    ...
    try:
        ...
        status = model_warmup_service.warmup_model(
            model, model_id, num_prompts=num_prompts, async_warmup=async_warmup
        )

        return jsonify(
            {
                "status": (
                    "warming" if async_warmup and not status.is_warmed else "warmed"
                ),
                "model_id": model_id,
                "is_warmed": status.is_warmed,
                "warmup_time_ms": (
                    status.warmup_time_ms if status.warmup_time_ms > 0 else None
                ),
                "kernel_compilation_time_ms": (
                    status.kernel_compilation_time_ms
                    if status.kernel_compilation_time_ms > 0
                    else None
                ),
                "error": status.error,
            }
        ...
        return jsonify({"error": "Warmup failed", "message": str(e)}), 500
        ...
        return jsonify(
            {
                ...
                "recommendation": (
                    "Use mmap"
                    if results.get("speedup", 0) > 1.2
                    else "Regular loading is fine"
                ),
            }
        )
